...
async function save() {
  ...
  if (saved && !disposed) await router.push({ name: 'study', params: { bookId: context.book.id, studyId: saved.id } })
}
...
<template>
  ...
  <div v-else-if="resourceState.error" class="notice error" role="alert">
    <h1>Não foi possível editar o estudo.</h1><p>{{ resourceState.error }}</p>
  ...
  <template v-else-if="resourceState.context">
    <nav class="breadcrumb" aria-label="Caminho">
      ...
      <RouterLink :to="{ name: 'book', params: { bookId: resourceState.context.book.id }, query: { chapter: resourceState.context.chapter.id } }">{{ resourceState.context.book.title }} · {{ resourceState.context.chapter.name }}</RouterLink>
    </nav>
    <header class="page-header"><p class="eyebrow">{{ resourceState.context.study.title }}</p><h1>Editar estudo</h1><p class="intro">Revise as seções e registre suas próprias interpretações.</p></header>
    <form class="import-form panel import-step" @submit.prevent="save">
      <fieldset :disabled="state.saving">
        <StudyEditorFields id-prefix="edit" show-metadata v-model:title="state.title" v-model:location="state.location" v-model:sections="state.sections" v-model:notes="state.notes" />
        ...
        <div class="save-bar">
          ...
          <div class="actions wrap">
            <RouterLink class="text-link" :to="{ name: 'study', params: { bookId: resourceState.context.book.id, studyId: resourceState.context.study.id } }">Cancelar</RouterLink>
            <button class="primary" :disabled="!canSave">{{ state.saving ? 'Salvando…' : 'Salvar alterações' }}</button>
          </div>
        </div>
      </fieldset>
    </form>
